...
|------|----------|------|----------|-----------------|
| `ai_tactical` | Pi / AI | 分钟至数小时 | 经筛选的高流动性币 | 持续扫描、提出中短线交易、动态管理仓位并复盘 |
| `user_thesis` | 用户给出核心方向，Pi / AI 提供信息验证与执行辅助 | 数小时至数天 | 优先 BTC / ETH，必要时 SOL | 保存 thesis、规划分批入场/退出、监视失效条件并维护灾难保护 |

基本边界：

- AI 的优势用于快速整合多币行情、盘口、OI、资金费率、新闻和宏观数据，不等于持续交易所有候选币。
- `ai_tactical` 只在结构和流动性足以覆盖手续费与风险时开仓。
- `user_thesis` 必须记录用户原始判断、允许加仓条件、失效条件、最大总仓位、最大风险和动态止盈/止损管理方案。
- 对 `user_thesis` 仓位，短周期噪音只能作为风险提示，不能单独触发与用户中长期判断冲突的反向交易或整仓退出。
- 两条轨道始终共享 Python 硬安全要求：受控执行、灾难止损、风险上限、日志和故障保护。
...
6. **风控闸门（AI-forward 模式）**
   ...
   - 开仓必须有 `disaster_stop` 或 `stop_loss`，以及 `invalid_if`；`take_profit` 可选。
   - `confidence` 与 RR 由 Pi 自主判断，Python 不以策略阈值替代 AI 决策。
   - 检查可用保证金，避免保证金不足。
   - 所有 live 执行仍由 Python 做最低限度硬校验。
   - 账户或订单读取不可信时拒绝破坏性操作与新开仓。
   - 支持在同一 symbol 现有同方向仓位上小额加仓；反向仓位、未成交入场单、非保护类条件单、缺少现有止损或会放松旧止损的加仓会被拒绝。
...
7. **执行能力**
   ...
   - 对已有同方向仓位，Pi 可继续用 `open_long` / `open_short` 表达加仓；成交后会按刷新后的总仓位替换保护止损。
   - 成交后自动挂 STOP_MARKET 灾难止损；只有 Pi 明确提供 `take_profit` 时才同时挂 TAKE_PROFIT_MARKET。
   - 默认仓位管理改为动态模式：Pi 每轮结合实时行情、新闻、OI、资金费率、盘口和趋势结构，主动决定继续持有、减仓、止盈、主动止损或收紧保护。
